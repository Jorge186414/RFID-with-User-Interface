...
Public Class Form1
    Dim Connection As New MySqlConnection("server=localhost; user=root; password=luffy1464; database=grupo_3401_proyecto_zamora")
    Dim MySQLCMD As New MySqlCommand
    Dim MySQLDA As New MySqlDataAdapter
    Dim DT As New DataTable
    Dim Table_Name As String = "datos_alumnos" 'your table name
    Dim Data As Integer
    ...
    Private Sub ShowData()
        ...
        Try
            If LoadImagesStr = False Then
                ...
                MySQLCMD.CommandText = "SELECT NOMBRE, ID, GRUPO, MATERIA, CORREO FROM " & Table_Name & " ORDER BY NOMBRE"
            ...
            Else
                MySQLCMD.CommandType = CommandType.Text
                MySQLCMD.CommandText = "SELECT FOTO FROM " & Table_Name & " WHERE ID LIKE '" & IDRam & "'"
                MySQLDA = New MySqlDataAdapter(MySQLCMD.CommandText, Connection)
                DT = New DataTable
                Data = MySQLDA.Fill(DT)
                If Data > 0 Then
                    Dim ImgArray() As Byte = DT.Rows(0).Item("FOTO")
    ...
    Private Sub ShowDataUser()
        Try
            Connection.Open()
        Catch ex As Exception
            MessageBox.Show("CONEXION FALLIDA !!!" & vbCrLf & "POR FAVOR VERIFICA QUE EL SERVIDOR ESTA LISTO !!!", "Error Message", MessageBoxButtons.OK, MessageBoxIcon.Error)
            Return
        End Try
        ...
        Try
            ...
            Data = MySQLDA.Fill(DT)
            If Data > 0 Then
                If LabelName.Text = "Esperando..." Then
                End If
                Dim ImgArray() As Byte = DT.Rows(0).Item("FOTO")
                Dim lmgStr As New System.IO.MemoryStream(ImgArray)
                ...
                LabelAddress.Text = DT.Rows(0).Item("GRUPO")
                LabelCity.Text = DT.Rows(0).Item("MATERIA")
                LabelCountry.Text = DT.Rows(0).Item("CORREO")
                SerialPort1.Write("1")
            Else
                SerialPort1.Write("0")
                MsgBox("ID NO ENCONTRADA !!!" & vbCr & "POR FAVOR REGISTRA TU  ID.", MsgBoxStyle.Information, "Information Message")
            End If
        Catch ex As Exception
            MsgBox("ERROR AL CARGAR LA BASE DE DATOS !!!" & vbCr & ex.Message, MsgBoxStyle.Critical, "Error Message")
            Connection.Close()
    ...
    Private Sub ClearInputUpdateData()
        ...
        TextBoxCity.Text = ""
        TextBoxCountry.Text = ""
        PictureBoxImageInput.Image = My.Resources.search_photo
    End Sub
    ...
    Private Sub ButtonUserData_Click(sender As Object, e As EventArgs) Handles ButtonUserData.Click
        If TimerSerialIn.Enabled = False Then
            MsgBox("ERROR AL ABRIR LA BASE DE DATOS !!!" & vbCr & "DA CLIC EN EL BOTON DE MENU CONEXION, Y REALIZA TU CONEXION.", MsgBoxStyle.Information, "Information")
            Return
        Else
            StrSerialIn = ""
            ViewUserData = True
    ...
    Private Sub ButtonScanPort_Click(sender As Object, e As EventArgs) Handles ButtonScanPort.Click
        ...
        Catch ex As Exception
            MsgBox("Com port NO DETECTADO", MsgBoxStyle.Critical, "Error Message")
            ComboBoxPort.Text = ""
            ComboBoxPort.Items.Clear()
            Return
        End Try
    ...
    Private Sub ButtonConnect_Click(sender As Object, e As EventArgs) Handles ButtonConnect.Click
        If ButtonConnect.Text = "Conectar" Then
            ...
            Try
                ...
                PictureBoxStatusConnectio.Image = My.Resources.status_connect
            Catch ex As Exception
                MsgBox("ERROR AL CONECTAR !!!" & vbCr & "No se detecto ningun Arduino.", MsgBoxStyle.Critical, "Error Message")
                PictureBoxStatusConnectio.Image = My.Resources.status_disconnect
            End Try
        ElseIf ButtonConnect.Text = "Desconectar" Then
            PictureBoxStatusConnectio.Image = My.Resources.status_disconnect
            ButtonConnect.Text = "Conectar"
            LabelConectionStatus.Text = "Estado de Conexion: Desconectado"
            TimerSerialIn.Stop()
            SerialPort1.Close()
        End If
    End Sub
    ...
    Private Sub ButtonClear_Click(sender As Object, e As EventArgs) Handles ButtonClear.Click
        ...
        LabelName.Text = "Esperando..."
        LabelAddress.Text = "Esperando..."
        LabelCity.Text = "Esperando..."
        LabelCountry.Text = "Esperando..."
    ...
    Private Sub ButtonSave_Click(sender As Object, e As EventArgs) Handles ButtonSave.Click
        ...
        If TextBoxName.Text = "" Then
            MessageBox.Show("Nombre no puede estar vacio !!!", "Error Message", MessageBoxButtons.OK, MessageBoxIcon.Error)
            Return
        End If

        If TextBoxAddress.Text = "" Then
            MessageBox.Show("Grupo no puede estar vacio !!!", "Error Message", MessageBoxButtons.OK, MessageBoxIcon.Error)
            Return
        End If

        If TextBoxCity.Text = "" Then
            MessageBox.Show("Materia no puede estar vacio !!!", "Error Message", MessageBoxButtons.OK, MessageBoxIcon.Error)
            Return
        End If

        If TextBoxCountry.Text = "" Then
            MessageBox.Show("Correo !!!", "Error Message", MessageBoxButtons.OK, MessageBoxIcon.Error)
            Return
        End If
        ...
        If StatusInput = "Save" Then
            ...
            Else
                MessageBox.Show("No se selcciono ninguna imagen !!!", "Error Message", MessageBoxButtons.OK, MessageBoxIcon.Error)
                Return
            End If
            Try
                Connection.Open()
            ...
            Try
                ...
                With MySQLCMD
                    .CommandText = "INSERT INTO " & Table_Name & " (NOMBRE, ID, GRUPO, MATERIA, CORREO, FOTO) VALUES (@name, @ID, @address, @city, @country, @images)"
                    ...
                    .ExecuteNonQuery()
                End With
                MsgBox("Datos guardados con exito", MsgBoxStyle.Information, "Information")
                IMG_FileNameInput = ""
                ClearInputUpdateData()
            Catch ex As Exception
                MsgBox("Error al guardar los datos !!!" & vbCr & ex.Message, MsgBoxStyle.Critical, "Error Message")
                Connection.Close()
                Return
            End Try
            Connection.Close()
        Else
            ...
            If IMG_FileNameInput <> "" Then
                ...
                Try
                    MySQLCMD = New MySqlCommand
                    With MySQLCMD
                        .CommandText = "UPDATE " & Table_Name & " SET  NOMBRE=@name,ID=@id,GRUPO=@address,MATERIA=@city,CORREO=@country,FOTO=@images WHERE ID=@id "
                        .Connection = Connection
                        .Parameters.AddWithValue("@name", TextBoxName.Text)
                        .Parameters.AddWithValue("@id", LabelGetID.Text)
                        .Parameters.AddWithValue("@address", TextBoxAddress.Text)
                    ...
                    ClearInputUpdateData()
                Catch ex As Exception
                    MsgBox("Error al actualizar los datos !!!" & vbCr & ex.Message, MsgBoxStyle.Critical, "Error Message")
                    Connection.Close()
                    Return
                End Try
                Connection.Close()
            Else
                ...
                Try
                    MySQLCMD = New MySqlCommand
                    With MySQLCMD
                        .CommandText = "UPDATE " & Table_Name & " SET  NOMBRE=@name,ID=@id,GRUPO=@address,MATERIA=@city,CORREO=@country WHERE ID=@id "
                        .Connection = Connection
                        .Parameters.AddWithValue("@name", TextBoxName.Text)
                        .Parameters.AddWithValue("@id", LabelGetID.Text)
                        .Parameters.AddWithValue("@address", TextBoxAddress.Text)
    ...
    Private Sub ButtonScanID_Click(sender As Object, e As EventArgs) Handles ButtonScanID.Click
        ...
        Else
            MsgBox("Error al abrir el menu de datos !!!" & vbCr & "DA CLIC EN EL BOTON DE MENU CONEXION, Y REALIZA TU CONEXION.", MsgBoxStyle.Critical, "Error Message")
        End If
    End Sub
    ...
    Private Sub TextBoxSearch_TextChanged(sender As Object, e As EventArgs) Handles TextBoxSearch.TextChanged
        If CheckBoxByID.Checked = True Then
            If TextBoxSearch.Text = Nothing Then
                SqlCmdSearchstr = "SELECT NOMBRE, ID, GRUPO, MATERIA, CORREO FROM " & Table_Name & " ORDER BY NOMBRE"
            Else
                SqlCmdSearchstr = "SELECT NOMBRE, ID, GRUPO, MATERIA, CORREO FROM " & Table_Name & " WHERE ID LIKE'" & TextBoxSearch.Text & "%'"
            End If
        End If
        If CheckBoxByName.Checked = True Then
            If TextBoxSearch.Text = Nothing Then
                SqlCmdSearchstr = "SELECT NOMBRE, ID, GRUPO, MATERIA, CORREO FROM " & Table_Name & " ORDER BY NOMBRE"
            Else
                SqlCmdSearchstr = "SELECT NOMBRE, ID, GRUPO, MATERIA, CORREO FROM " & Table_Name & " WHERE NOMBRE LIKE'" & TextBoxSearch.Text & "%'"
            End If
        End If
        ...
        Try
            ...
            End If
        Catch ex As Exception
            MsgBox("Error al buscar" & vbCr & ex.Message, MsgBoxStyle.Critical, "Error Message")
            Connection.Close()
        End Try
        Connection.Close()
    End Sub
    ...
    Private Sub TimerSerialIn_Tick(sender As Object, e As EventArgs) Handles TimerSerialIn.Tick
        Try
            ...
            LabelConectionStatus.Text = "Estado de Conexion: Conectado"
            If StrSerialIn <> "" Then
                If GetID = True Then
        ...
        If PictureBoxStatusConnectio.Visible = True Then
            PictureBoxStatusConnectio.Visible = False
        ElseIf PictureBoxStatusConnectio.Visible = False Then
            PictureBoxStatusConnectio.Visible = True
        End If
    End Sub
    ...
    Private Sub IDCheck()
        ...
        Try
            ...
            Data = MySQLDA.Fill(DT)
            If Data > 0 Then
                If MsgBox("ID registrada !" & vbCr & "Quieres editar los datos ?", MsgBoxStyle.Question + MsgBoxStyle.OkCancel, "Confirmation") = MsgBoxResult.Cancel Then
                    DT = Nothing
                    Connection.Close()
                ...
                Else
                    ...
                    TextBoxAddress.Text = DT.Rows(0).Item("GRUPO")
                    TextBoxCity.Text = DT.Rows(0).Item("MATERIA")
                    TextBoxCountry.Text = DT.Rows(0).Item("CORRE0")
                    StatusInput = "Update"
                End If
            End If
        Catch ex As Exception
    ...
    Private Sub PanelHead_Resize(sender As Object, e As EventArgs) Handles PanelHead.Resize
        PanelHead.Invalidate()
    End Sub
End Class